         ...
         )(implicit p: Parameters) extends BoomModule()(p)
{
   ...
   for (w <- 0 until width)
   {
      ...
      when (io.enq_valids(w))
      {
         ...
         if (!(io.enq_uops(w).is_store || io.enq_uops(w).is_load || io.enq_uops(w).is_amo || io.enq_uops(w).is_fencei || io.enq_uops(w).is_fence || io.enq_uops(w).is_jmp || io.enq_uops(w).flush_on_commit)){
