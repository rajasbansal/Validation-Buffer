         ...
         )(implicit p: Parameters) extends BoomModule()(p)
{
   ...
   for (w <- 0 until width)
   {
      ...
      when (io.enq_valids(w))
      {
         ...
         rob_validated(rob_tail) := !(io.enq_uops(w).is_store || io.enq_uops(w).is_load || io.enq_uops(w).is_amo || io.enq_uops(w).is_fencei || io.enq_uops(w).is_fence || io.enq_uops(w).is_br_or_jmp || io.enq_uops(w).flush_on_commit || io.enq_uops(w).is_unique || ( io.enq_uops(w).fu_code === UInt(2) ) || (io.enq_uops(w).uopc === uopADDI && io.enq_uops(w).pop2 === UInt(0)))
      ...
      for (i <- 0 until num_wakeup_ports)
      {
         ...
         when (wb_resp.valid && MatchBank(GetBankIdx(wb_uop.rob_idx)) && rob_val(row_idx))
         {
...
//                  "FP instruction writing back exc bits is overriding an existing exception.")
      ...
      // Stores have a separate method to clear busy bits (Removing this because we deal with stores through validated signal)
      ...
      rob_head_is_branch(w):= rob_uop(rob_head).is_br_or_jmp
