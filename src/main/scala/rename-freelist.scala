   ...
   for (w <- 0 until 3*pl_width)
   {
      when (io.pending_readers_vals(w))
      {
       ...
       //   }
         ...
         pending_readers_list(io.pending_readers_regs(w)) := pending_readers_list(io.pending_readers_regs(w)) + ((io.pending_readers_regs zip io.pending_readers_vals) map {case (v,val_bit) => ((v === io.pending_readers_regs(w)) && (val_bit))}).count({case (v) => v === true})
